...
export class SyncPayloadsService {
  ...
  /**
   * Compute the hash of an environment.
   *
   * @param environment
   * @returns
   */
  public computeHash(environment: Environment) {
    return from(
      MainAPI.invoke('APP_GET_HASH', deterministicStringify(environment))
    );
  }
    ...
    if (syncAction.type === SyncActionTypes.ADD_CLOUD_ENVIRONMENT) {
      ...
      if (
        ...
        this.environmentsService
          .addEnvironment(
            {
              environment: syncAction.environment,
              promptSave: false,
              cloud: true
            },
